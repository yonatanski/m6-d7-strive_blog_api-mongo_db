...
import UsersModel from "../authors/schema.js"
...
const googleStrategy = new GoogleStrategy(
  ...
  async (accessToken, refreshToken, profile, passportNext) => {
    try {
      ...
      const user = await AuthorModel.findOne({ email: profile.emails[0].value })
      ...
      if (user) {
        ...
        const token = await authenticateUser(saveAuthor)
