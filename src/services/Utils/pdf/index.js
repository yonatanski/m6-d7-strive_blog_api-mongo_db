...
export const generatePDFAsync = async (data) => {
  ...
  const asyncPipeline = promisify(pipeline) /* promisify is a (VERY COOL) function from 'util' core module, which transforms
   a function that uses callbacks (error-first callbacks) into a function that uses Promises (and so Async/Await). 
   Pipeline is a function that works error-first callbacks --> I can promisify a pipeline, getting back an "asynchronous pipeline"  */
